...
import os
import sys
cur_dir = '/'.join(os.path.dirname(__file__).split('/')[:-2])
cur_dir = os.path.join(cur_dir, 'util')
...
def accuracy(logits, y, args):
    _, preds = torch.max(logits, 1)
    # for i in range(len(y)):
    #     tmp_y = args.idx_to_class.get(y[i].item())
    #     tmp_preds = args.idx_to_class.get(preds[i].item())
    #     comp_info = 'status:%s,epoch:%s,y:%s,predict_y:%s\n'%(args.status, args.cur_epoch, tmp_y, tmp_preds)
    #     log_recording(args.predict_log, comp_info, 'a+')

        # if tmp_y != tmp_preds:
        #     if args.status == 'train':
        #         if len(args.dist_list_train['diff']) > 10:
        #             continue
        #         args.dist_list_train['diff'].append('%s-%s'%(tmp_y, tmp_preds))
        #     else:
        #         if len(args.dist_list_test['diff']) > 10:
        #             continue
        #         args.dist_list_test['diff'].append('%s-%s'%(tmp_y, tmp_preds))
        # else:
        #     if args.status == 'train':
        #         if len(args.dist_list_train['same']) > 10:
        #             continue
        #         args.dist_list_train['same'].append('%s-%s'%(tmp_y, tmp_preds))
        #     else:
        #         if len(args.dist_list_test['same']) > 10:
        #             continue
        #         args.dist_list_test['same'].append('%s-%s'%(tmp_y, tmp_preds))
    return (preds == y).float().mean()

def pass_epoch(
    model, loss_fn, loader, optimizer=None, scheduler=None,
    batch_metrics={'time': BatchTimer()}, show_running=True,
    device='cpu', writer=None, args=None
):
    ...
    for i_batch, (x, y) in enumerate(loader):
        ...
        for metric_name, metric_fn in batch_metrics.items():
            if metric_name == 'acc':
                metrics_batch[metric_name] = metric_fn(y_pred, y, args).detach().cpu()
            else:
